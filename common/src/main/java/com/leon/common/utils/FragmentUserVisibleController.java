...
public class FragmentUserVisibleController {
    ...
    public void activityCreated() {
        if (DEBUG) {
            LogUtils.tag(TAG).d(fragmentName + ": activityCreated, userVisibleHint=" + fragment.getUserVisibleHint());
        }
        if (fragment.getUserVisibleHint()) {
            Fragment parentFragment = fragment.getParentFragment();
            if (parentFragment != null && !parentFragment.getUserVisibleHint()) {
                if (DEBUG) {
                    LogUtils.tag(TAG).d(fragmentName + ": activityCreated, parent " + parentFragment.getClass().getSimpleName() + " is hidden, therefore hidden self");
                }
    ...
    public void resume() {
        if (DEBUG) {
            LogUtils.tag(TAG).d(fragmentName + ": resume, userVisibleHint=" + fragment.getUserVisibleHint());
        }
        if (fragment.getUserVisibleHint()) {
            userVisibleCallback.onVisibleToUserChanged(true, true);
            callbackListener(true, true);
            if (DEBUG) {
                LogUtils.tag(TAG).i(fragmentName + ": visibleToUser on resume");
            }
        }
    }
    ...
    public void pause() {
        if (DEBUG) {
            LogUtils.tag(TAG).d(fragmentName + ": pause, userVisibleHint=" + fragment.getUserVisibleHint());
        }
        if (fragment.getUserVisibleHint()) {
            userVisibleCallback.onVisibleToUserChanged(false, true);
            callbackListener(false, true);
            if (DEBUG) {
                LogUtils.tag(TAG).w(fragmentName + ": hiddenToUser on pause");
            }
        }
    }
    ...
    public void setUserVisibleHint(boolean isVisibleToUser) {
        ...
        if (DEBUG) {
            ...
            LogUtils.tag(TAG).d(fragmentName + ": setUserVisibleHint, userVisibleHint=" + isVisibleToUser + ", " + (fragment.isResumed() ? "resume" : "pause") + ", " + parent);
        }
        ...
        if (isVisibleToUser) {
            if (parentFragment != null && !parentFragment.getUserVisibleHint()) {
                if (DEBUG) {
                    LogUtils.tag(TAG).d( fragmentName + ": setUserVisibleHint, parent " + parentFragment.getClass().getSimpleName() + " is hidden, therefore hidden self");
                }
                userVisibleCallback.setWaitingShowToUser(true);
                userVisibleCallback.callSuperSetUserVisibleHint(false);
                return;
        ...
        if (fragment.isResumed()) {
            ...
            if (DEBUG) {
                if (isVisibleToUser) {
                    LogUtils.tag(TAG).i(fragmentName + ": visibleToUser on setUserVisibleHint");
                } else {
                    LogUtils.tag(TAG).w(fragmentName + ": hiddenToUser on setUserVisibleHint");
                }
            }
        }
                    ...
                    for (Fragment childFragment : childFragmentList) {
                        if (childFragment instanceof UserVisibleCallback) {
                            ...
                            if (userVisibleCallback.isWaitingShowToUser()) {
                                if (DEBUG) {
                                    LogUtils.tag(TAG).d(fragmentName + ": setUserVisibleHint, show child " + childFragment.getClass().getSimpleName());
                                }
                    ...
                    for (Fragment childFragment : childFragmentList) {
                        if (childFragment instanceof UserVisibleCallback) {
                            ...
                            if (childFragment.getUserVisibleHint()) {
                                if (DEBUG) {
                                    LogUtils.tag(TAG).d(fragmentName + ": setUserVisibleHint, hidden child " + childFragment.getClass().getSimpleName());
                                }
                                userVisibleCallback.setWaitingShowToUser(true);
